...
function respond(row: TweetCheckRow) {
  return {
    posted: row.found === 1,
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const account = await env.DB.prepare(
    'SELECT twitter_handle FROM user_twitter_accounts WHERE wallet_address = ?',
  )
    .bind(walletAddress)
  ...
  const code = await env.DB.prepare(
    "SELECT code_slug FROM referral_codes WHERE wallet_address = ? AND status = 'active' ORDER BY id DESC LIMIT 1",
  )
    .bind(walletAddress)
    .first<ReferralCodeRow>()

  if (!code) {
    return Response.json({ error: 'no_active_referral_code' }, { status: 409 })
  }

  const handle = account.twitter_handle
  const codeSlug = code.code_slug

  const cached = await env.DB.prepare(
    'SELECT found, tweet_id, tweet_url, tweet_created_at, checked_at FROM referral_tweet_checks WHERE wallet_address = ? AND code_slug = ?',
  )
    .bind(walletAddress, codeSlug)
    .first<TweetCheckRow>()
  ...
  let searchResult
  try {
    searchResult = await searchTweetsByUser({ apiKey, handle, query: codeSlug })
  } catch (error) {
    if (error instanceof TwitterApiError) {
      console.error('twitterapi.io error', error)
      return Response.json(
        { error: 'twitter_api_error', status: error.status },
        { status: 502 },
      )
    }
  ...
  await env.DB.prepare(
    `INSERT INTO referral_tweet_checks
       (wallet_address, code_slug, twitter_handle, found, tweet_id, tweet_url, tweet_created_at, checked_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
     ON CONFLICT(wallet_address, code_slug) DO UPDATE SET
    ...
    .bind(walletAddress, codeSlug, handle, found, tweetId, tweetUrl, tweetCreatedAt)
    .run()
